...
class YOLO(object):
    def __init__(self, architecture,
        ...
        output = Reshape((int(self.grid_h), int(self.grid_w), int(self.nb_box), int(4 + 1 + self.nb_class)))(output)
    ...
    def custom_loss(self, y_true, y_pred):
        ...
        cell_grid = tf.tile(tf.concat([cell_x,cell_y], -1), [self.batch_size, 1, 1, int(self.nb_box), 1])
        ...
        ### adjust w and h
        pred_box_wh = tf.exp(y_pred[..., 2:4]) * np.reshape(self.anchors, [1,1,1,int(self.nb_box),2])
        ...
        true_box_xy, true_box_wh, coord_mask = tf.cond(tf.less(seen, self.warmup_bs), 
                              lambda: [true_box_xy + (0.5 + cell_grid) * no_boxes_mask, 
                                       true_box_wh + tf.ones_like(true_box_wh) * np.reshape(self.anchors, [1,1,1,int(self.nb_box),2]) * no_boxes_mask,
